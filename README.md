...
| 回忆收藏馆 / 结局收集 | P1 | 待实现 | — |
| 音频（BGM / 音效 / 静音） | P2 | 已具备 | `@plugin(audio)`：多通道（bgm/se）循环与一次性播放、停/暂停/继续/音量；节点 `audio` 属性与 `music` 节点类型已删除 |
| 系统提示（toast 提示条） | P2 | 已具备 | `type = toast` 节点：自带默认样式的提示条（右上角“存档中…”），显示 `duration` 秒后自动淡出 |
| 设置界面 / CG 收集 | P2 | 待实现 | — |
...
| duration | 系统提示（toast）停留秒数 | 不写 = 默认 2.2 秒，之后自动淡出 |
...
- 画布左上角**常驻工具箱**：把「文本 / 文本框 / 按钮 / 背景 / 立绘 / 人物名 / 对话 / 系统提示」拖到画布即生成节点（单击也可在可视中心放置；系统提示会直接摆到右上角）。
