...
- **免安装试玩版**：[Releases](https://github.com/liceses/ds-adventure-game/releases) —— Windows x64，解压后双击 `ds-adventure.exe`，目标机器**无需 JDK**（当前 `v0.3.0-demo`：序章 + 第1章 + 第2章；含演出优化、五子棋 / 推箱子、真实背景与立绘）
...
| 倒计时时钟（示例地图） | P2 | 已具备 | `maps/demo_clock`：倒计时 / 正常时钟双模式、可调 1~60 分、到点自动跳「时间到」、未到点走「提前离开」（两条分支不同场景）；演示场景自动信号 + `after/every/stoptimer` 定时器 |
| 节点信号 / 槽总开关 | P2 | 已具备 | `signalsEnabled` / `slotsEnabled` 属性 + `@plugin(switch)`：演出期间锁交互、选项只能点一次 |
| 设置界面 / CG 收集 | P2 | 待实现 | — |
...
| 终局（`final`） | 终章 | **待实现** | 终章 `@minigame final mode:ending` 的玩法口径待剧情侧给；编译已通过，运行到该处会提示缺插件 |
...
| 键 | 含义 | 备注 |
|---|---|---|
| type | bg背景 \| char立绘 \| text文本 \| name人物名 \| dialog对话 \| button按钮 \| textbox文本框 \| toast系统提示 | 必填 |
| x / y | 逻辑坐标（画布 1280×720） | |
| width / height | 显示尺寸 | 缺省 = 类型默认 |
| path | 图片 / 立绘相对路径（相对地图根） | 缺失时自动显示“占位渐变图” |
| video | 视频路径 | 设了就代替图片渲染该节点（背景节点＝会动的背景图） |
...
| signalsEnabled / slotsEnabled | **信号 / 槽总开关**（中文别名 信号开关 / 槽开关） | 默认都开，只在关掉时落盘；运行时用 `@plugin(switch)` 改 |
| typewriter / 逐字显示 | 对话逐字开关：默认 / 开 / 关（对话默认开） | 逐字时点击对话 = 显示全文 |
| 其它任意键 | 存入 extras 原样往返 | 保证无损同步 |

**节点的信号 / 槽总开关**（`signalsEnabled` / `slotsEnabled`，默认都开）：

```ini
{ type = button  id = 存档按钮  text = 存档
  signalsEnabled = false     ; 这个按钮点了不会有反应（信号不发出）
  slotsEnabled   = false     ; 挂在这个节点上的槽也不执行（场景级槽不受影响）
}
```

- 「信号关」= 该节点的鼠标 / 键盘 / 按钮信号不再发出（也含槽里以它名义 `emit` 的信号）；
  「槽关」= 挂在**本节点上**的槽不再执行 —— 场景级槽照常。
- 运行时随时改（自带插件 `@plugin(switch)`，别名 开关 / 节点开关 / 启用 / 禁用）：
  `@plugin(switch) | on|off|toggle | 节点id | signals|slots|all`
  ```ini
  slot = 演出开始 | @plugin(switch) | off    | 按钮_存档 | signals   ; 过场期间锁住交互
  slot = 演出结束 | @plugin(switch) | on     | 按钮_存档 | signals
  slot = 点过一次 | @plugin(switch) | off    | 选项_A    | all       ; 选项只能点一次
  ```
- 编辑器：节点属性窗口有「启用信号 / 启用槽」复选框；画布上被关掉的节点带角标（🔇 信号关 / ⛔ 槽关）。

### 引擎自动信号（场景切换，不用声明）

场景切换时读取器自动发出三个信号（写槽就能收到，`@param(名)` 取参数）：

| 信号（别名） | 什么时候发 | 参数 | 典型用途 |
|---|---|---|---|
| `场景进入`（进入场景 / 场景开始） | 新场景渲染完成后 | `scene`=当前场景　`from`=上一个场景名（第一幕为空） | 初始化变量、放 BGM、起定时器、自动存档 |
| `场景离开`（离开场景 / 场景结束） | 即将切走、旧场景还没被清掉时 | `scene`=被离开的场景　`to`=即将进入的场景 | 收尾演出、存状态、**停掉定时器** |
| `上一个场景离开`（上一幕离开 / 上个场景离开） | 新场景渲染完成后（紧跟「场景进入」） | `scene`=被离开的场景　`to`=当前场景 | 在新场景里对“刚走的那一幕”做反应（记录来路、接续上一幕的倒计时）|

```
[时钟]
slot = 场景进入 | @plugin(every) | 1 | 每秒          ← 进场景就起一个每秒定时器
slot = 每秒     | @plugin(clock) | now | @var(时间)   ← 定时器发信号 → 槽里干活
slot = 场景离开 | @plugin(stoptimer) | 每秒          ← 换幕时停掉它
```

槽里“定时器 → 发信号”用自带插件就能做：`@plugin(after) | N | 信号名`（N 秒后发一次）、
`@plugin(every) | N | 信号名`（每 N 秒）、`@plugin(stoptimer) | 信号名`（停表）。
完整可运行示例：**文件 → 🎁 生成演示地图 → 倒计时时钟**（`maps/demo_clock`）。
...
- **左侧层级树点一下就选中**：点节点 → 切到它所在场景、画布上出现选中样式（高亮圈画在节点内容**之上**，
  不会被内容盖住），并**自动把它滚进可视区**（画布放大 / 平移过也看得见）；右侧检查器同时刷新出该节点。
  点场景一行则切场景。
- 画布左上角**常驻工具箱**：把「文本 / 文本框 / 按钮 / 背景 / 立绘 / 人物名 / 对话 / 系统提示」拖到画布即生成节点（单击也可在可视中心放置；系统提示会直接摆在右上角）。
