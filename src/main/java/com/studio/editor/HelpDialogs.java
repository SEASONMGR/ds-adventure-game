...
final class HelpDialogs {
    ...
    private static String flowHelp() {
        return """
                ...
                ============ 五、插件 ============
                """ + com.studio.plugin.builtin.BuiltinCatalog.manual() + """
                ...
                引擎自动信号（不需要声明 signal，直接写槽就行）：
                  slot = 场景进入 | …   ← 每次进入本场景时自动执行（参数 scene=场景名）
                  slot = 场景离开 | …   ← 离开本场景前自动执行（此时旧场景节点还在）
                  常见用法：进场景自动存档、自动刷新派生变量、自动播一段演出。
    ...
    private static String shortcutHelp() {
        return """
                ...
                拖动节点             改坐标（一次拖动 = 一步撤销）
                按住右键拖动画布     平移视窗 —— 放大后地图比可视区大，用它把四周的角落拖进来看
                                     （中键拖动同样可以；单击右键不动仍是弹菜单，两者不会打架）
                                     Ctrl+0 适应窗口会同时复位缩放与平移
                ...
                ============ 快捷入口 ============
                文件 → 📂 打开地图…（列表）                        一张窗口看全默认文件夹里的地图：
                                                                  场景/节点数、修改时间、路径；双击即开
                                                                  同一窗口里还有「新建地图」「生成演示地图」
                把地图文件夹拖进窗口                                直接拖文件夹（或 scenario.txt）到编辑器窗口即打开
                                                                  （拖整个 maps/ 这种上层目录也行：只有一张地图就直接开，
                                                                    有多张会打开列表窗口让你挑）
                编辑 → 🗂 外部编辑（复制当前地图到临时目录并打开）…  把地图复制到「external-edit/」工作副本，
                                                                  用别的工具改 scenario.txt，改完导回来
                编辑 → 📥 导入外部更改（覆盖原地图）…               列出改动文件并确认，覆盖前自动备份原地图
                编辑 / 画布右键 → ⭐ 个性化节点                      把调好的节点存成模板（要命名），
                                                                  之后在「⭐ 个性化节点列表」里一键套用到新节点
                                                                  全部模板存在工程根目录的 node-presets.txt
                保存地图（Ctrl+S）                                  手写的 # 注释会被保住：文件头说明、[option]、
                                                                  场景上方、节点上方、属性行上方、文件末尾的注释
                                                                  都会按“挂在谁头上”合并回新文件；场景改名/节点换 id
                                                                  会按位置兜底匹配；确实没落点的会提示“N 处注释没有落点”
